"""
YouTube Upload Script - VELOCITY INDONESIAN
"""
import os
...
def generate_video_metadata(category, num_phrases, phrases=None):
    title = f"Indonesian Learning: {num_phrases} Essential {category} Phrases"
    lines = [f"Learn Indonesian with VELOCITY INDONESIAN!", "", f"Category: {category}", "", f"Master Indonesian one phrase at a time! Today's {category} lesson:", ""]
    if phrases:
        for i, p in enumerate(phrases[:5], 0):
            lines.append(f"{i+1}. {p['english']}")
            lines.append(f"   {p.get('indonesian', '')}")
            lines.append(f"   [{p.get('transliteration', '')}]")
            lines.append("")
    lines.extend(["Tip: Repeat each phrase out loud 3 times!", "Like this video if you learned something new!", "Subscribe for daily Indonesian lessons!", "", f"#LearnIndonesian #IndonesianLessons #IndonesianForBeginners #LanguageLearning", f"#Indonesian #Education #Tutorial #DailyIndonesian #{category.replace(' ', '')}", f"#VELOCITYINDONESIAN #IndonesianPhrases #SpeakIndonesian"])
    tags = [f"learn indonesian", f"indonesian lessons", f"indonesian for beginners", f"indonesian phrases", "language learning", f"speak indonesian", category.lower(), "education", f"daily indonesian", "velocity indonesian", f"indonesian learning"]
    return title, "\n".join(lines), tags

def upload_to_youtube(video_path, title, description, tags=None, category_id='22'):
    if tags is None: tags = ['education', 'language learning', '#Indonesian']
...
def main():
    ...
    try: upload_to_youtube(video_path=video_file, title=f"Learn Indonesian Daily", description=f"#shorts #learnindonesian #indonesian #language #education", tags=['#Indonesian', 'education', 'language learning'], category_id='22')
